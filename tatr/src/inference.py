...
def get_args():
    ...
    parser.add_argument('--mode',#recognize
...
def main():
    ...
    # Load images
    img_files = [item for item in os.listdir(args.image_dir) if item.endswith('.png')]
    ...
    for count, img_file in enumerate(img_files):
        ...
        if args.mode == 'recognize':
            ...
            with open(os.path.join(args.out_dir, 'predicted.json'), 'w') as f:
                json.dump(cells_info, f, indent=4)
        ...
        if args.mode == 'detect':
            pred, detected_tables = pipe.detect(img, tokens, out_objects=args.objects, out_crops=args.crops)
            print(img_file, ':', pred)

            save_detect_pred[img_file] = pred
            ...
            for table_idx, extracted_table in enumerate(extracted_tables):
                for key, val in extracted_table.items():
                    output_result(key, val, args, extracted_table['image'],
                                  img_file.replace('.png', '_{}.png'.format(table_idx)))
                    
        
    with open(os.path.join(args.out_dir, 'pred.json'), 'w') as f:
        json.dump(save_detect_pred, f, indent=4)
