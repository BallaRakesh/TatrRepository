...
def train(args, model, criterion, postprocessors, device):
    ...
    track_ap = [0] *( args.epochs - args.start_epoch)
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print(f"GPU utilization: {gpu.used //1024**2} MB")
        ...
        track_ap[epoch] = round(float(pubmed_stats['coco_eval_bbox'][0]), 2)
        
        if (epoch == 0) or (track_ap[epoch] > track_ap[epoch-1]):
            
            #all_models = os.listdir(output_directory)
            if epoch != 0:
                prev_best_model = glob.glob(os.path.join(output_directory, 'best_model_*'))
                if len(prev_best_model) == 1:
                    os.remove(prev_best_model[0])
                elif len(prev_best_model) > 1:
                    for prev_model in prev_best_model:
            ...
            if (epoch+1) % args.checkpoint_freq == 0:
                model_save_path_epoch = os.path.join(output_directory, f'best_model_ap_{track_ap[epoch]}_{str(epoch+1)}.pth')
