...
def main():
    ...
    for file_idx, filename in tqdm(enumerate(pred_filenames), 'Processing'):
        ...
        if not pred_bboxes == pred_labels == 0:
            ...
            row_texts = [get_text_area(pred_rows[i], ocr_words) for i in range(len(pred_rows))]
            col_texts = [get_text_area(pred_columns[i], ocr_words) for i in range(len(pred_columns))]
            ...
            xml_object = generate_xml(xmlname=xml_name, 
                                    table_w=table_w, 
                                    table_h=table_h, 
