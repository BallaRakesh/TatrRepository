...
def generate_train_val_split(train_txt_path, val_split_ratio=0.15):
    ...
    with open('complex_imgs.txt', 'r') as f:
        complex_tables = f.readlines()
    complex_tables = [item.replace('\n','') for item in complex_tables]
    
    all_train_files_pattern = [item[-8:] for item in all_train_files]
    non_complex_images = []
    for i, img in enumerate(all_train_files_pattern):
        if img not in complex_tables:
            non_complex_images.append(all_train_files[i])
    
    train_max_idx = int(len(non_complex_images) * (1 - val_split_ratio))
    
    train_files = non_complex_images[:train_max_idx]
    val_files = non_complex_images[train_max_idx:]
