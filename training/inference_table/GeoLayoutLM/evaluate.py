...
def main():
    ...
    # LOAD MODEL :: path to model files
    ...
    if cfg.model.backbone in [
        ...
        "alibaba-damo/geolayoutlm-large-uncased",  ## hardcode to this
    ]:
...
def visualize_tagging(detail_path):
    file_paths = glob(os.path.join(detail_path, "*_tagging.json"))
    vis_dir = os.path.join(detail_path, 'vis_labeling')
    final_data_dir = os.path.join(detail_path, 'predicted_labels')
    ...
    for fp in tqdm(file_paths):
        ...
        tagging_file = fp.replace('json', 'txt')
        with open(tagging_file, 'r') as tf:
            img_path = tf.readline().strip('\n')
        ...
        for entry in json_data:
            if 'data_cell' in entry['pred_key']:
                bbox_label = 'data'
                color_box = (0, 0, 244)
            elif 'header_cell' in entry['pred_key']:
                color_box = (244, 0, 0)
                bbox_label = 'header'
            elif 'trash' in entry['pred_key']:
                color_box = (0,0,0)
                bbox_label = 'trash'
            else:
                color_box = (0,0,0)
            ...
            # if entry['coords'] not in coords:
            #     coords.append(entry['coords'])
            row_data.append({'coords': entry['coords'], 'label': bbox_label})
                
            cv2.rectangle(img, (entry['coords'][0], entry['coords'][1]), 
                                (entry['coords'][2], entry['coords'][3]), 
                          color_box, 2)
        del coords
        cv2.imwrite(os.path.join(vis_dir, img_path.split('/')[-1]), img)
        jsonname = img_path.split('/')[-1].replace('png', 'json')
        with open(os.path.join(final_data_dir, jsonname), 'w') as f:
            json.dump({'prediction':row_data}, f, indent=4)
