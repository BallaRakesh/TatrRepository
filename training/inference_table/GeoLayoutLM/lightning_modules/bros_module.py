...
class BROSModule(LightningModule):
    ...
    def _log_shell(self, log_info, prefix=""):
        ...
        dic = {self.current_epoch :{}}
        for key, value in log_info_shell.items():
            out_str += f" || {key}: {round(value, 5)}"
            self#print(f" || {key}: {round(value, 5)}",flush=True) 
            dic[self.current_epoch].update({key: round(value, 5)})
        self.loss_dict_per_epoch.append(dic)
        
        if self.cfg.train.max_epochs - 1 == self.current_epoch:
            if not os.path.exists(self.cfg.metric_dump_path):
                os.makedirs(self.cfg.metric_dump_path)
            file_path = os.path.join(self.cfg.metric_dump_path, "loss.json") 
            with open(file_path, 'w') as file:
                json.dump(self.loss_dict_per_epoch, file)
