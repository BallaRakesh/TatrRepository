...
python3 tatr.py --image_dir ../images --out_dir ../tatr_out --structure_config_path configs/tatr_structure_config.json --structure_model_path models/finetuned/tatr_best_model_ap_0.68_35.pth --objects --csv --cells --html
...
import inference.postprocess.tatr
# sys.path.append("detr")
sys.path.append("./inference/detr")
...
def load_config(config_path='config.ini'):
    ...
    return ConfigParser(config['DEFAULT'])
...
def cells_to_html(cells):
    ...
    return table_cells, str(ET.tostring(table, encoding="unicode", short_empty_elements=False))
...
class TableExtractionPipeline(object):
    ...
    def detect(self, img, tokens=None, out_objects=True, out_crops=True, crop_padding=10):
        out_formats = {}
        ...
        return objects, out_formats

    def recognize(self, img, tokens=None, out_objects=True, out_cells=False,
        ...
        if out_html:
            ...
            for cells in tables_cells:
                table_cell, tables_html = cells_to_html(cells)
                cells_data.extend(table_cell)
                tables_htmls.append(tables_html)
            # table_cells, tables_htmls = [cells_to_html(cells) for cells in tables_cells]
            out_formats['html'] = tables_htmls
            out_formats['table_structure'] = tables_structure
    ...
    def extract(self, img, tokens=None, out_objects=True, out_crops=False, out_cells=False,
        ...
        pred, detect_out = self.detect(img, tokens=tokens, out_objects=False, out_crops=True,
        ...
        for table in cropped_tables:
            ...
            extracted_table[1]['image'] = img
            extracted_table[1]['tokens'] = tokens
            extracted_tables.append(extracted_table[1])
...
def output_result(key, val, args, img, img_file):
    ...
    if key == 'table_structure':
        out_file = img_file.replace(".png", "_structure.json")
        with open(os.path.join(args.out_dir, out_file), 'w') as f:
            json.dump(val, f, indent=4)
    elif key == 'objects':
        if args.verbose:
            print(val)
        out_file = img_file.replace(".png", "_objects.json")
        row_data = img_file.replace(".png", "_rows.json")
        col_data = img_file.replace(".png", "_cols.json")
        rows = [item for item in val if item['label'] == 'table row']
        cols = [item for item in val if item['label'] == 'table column']
        with open(os.path.join(args.out_dir, out_file), 'w') as f:
            json.dump(val, f, indent=4)
        with open(os.path.join(args.out_dir, row_data), 'w') as f:
            json.dump(rows, f, indent=4)
        with open(os.path.join(args.out_dir, col_data), 'w') as f:
        ...
        for idx, elem in enumerate(val):
            if key == 'crops':
                for idx, cropped_table in enumerate(val):
                    out_img_file = img_file.replace(".png", "_table_{}.png".format(idx))
                    cropped_table['image'].save(os.path.join(args.out_dir,
                                                                out_img_file))
                    out_words_file = out_img_file.replace(".png", "_words.json")
                ...
                out_file = img_file.replace(".png", "_{}.{}".format(idx, key))
                with open(os.path.join(args.out_dir, out_file), 'w') as f:
                    if elem == None: elem = ''
                    f.write(elem)
                if args.verbose:
...
def load_structure_model(args, device):
   
    # Create inference pipeline
    # #print("Creating inference pipeline")
    pipe = TableExtractionPipeline(det_device=device,
                                   str_device=device,
...
def run_prediction(pipe,
    ...
    # if args.words_dir != '':
    #     tokens_path = os.path.join(args.words_dir, img_file.replace(".png", "_words.json"))
    ...
    if args.mode == 'recognize':
        ...
        cells_data, extracted_table = pipe.recognize(img, tokens, out_objects=args.objects, out_cells=args.csv,
                            out_html=args.html, out_csv=args.csv)
        
        # cells_info[img_file] = cells_data

        # #print("Table(s) recognized.")

        # #print()
        rows = [item for item in extracted_table['objects'] if item['label'] == 'table row']
